...
PostProcessor::PostProcessor(int screenWidth, int screenHeigth)
{
	...
	shaders.push_back(tdogl::Shader::shaderFromFile(("postProcessor.vs"), GL_VERTEX_SHADER));
	shaders.push_back(tdogl::Shader::shaderFromFile(("postProcessor.fs"), GL_FRAGMENT_SHADER));
...
{
	glDeleteRenderbuffers(1, &m_iRbo_depth);
...
void PostProcessor::initializeFBO()
{
	...
	glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT32, m_iScreenWidth, m_iScreenHeigth);
	...
	glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, m_iRbo_depth);
